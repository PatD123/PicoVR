...
static inline vec3_t cross(const vec3_t *const v0, const vec3_t *const v1)
{
    return (vec3_t){
        v0->y * v1->z - v0->z * v1->y,
        v0->z * v1->x - v0->x * v1->z,
...
static inline float mag3(const vec3_t *const v)
{
    return sqrtf(v->x * v->x + v->y * v->y + v->z * v->z);
}

static inline vec3_t normalize(const vec3_t *const v)
{
    float v_mag = mag3(v);
    return (vec3_t){
        v->x / v_mag,
